...
def copy_workflows():
    ...
    try:
        ...
        if not workflows_src.exists():
            print("[MotionCapture] No workflows/ directory found, skipping workflow copy")
            return
        ...
        for item in workflows_src.iterdir():
            ...
            # Destination path with GVHMR- prefix
            dest = workflow_dir / f"GVHMR-{item.name}"
            ...
            try:
                shutil.copy2(item, dest)
                print(f"[MotionCapture] Copied workflow: {item.name} -> {dest}")
                copied_count += 1
            except Exception as e:
                print(f"[MotionCapture] Failed to copy {item.name}: {e}")
        ...
        # Print summary
        if copied_count > 0:
            print(f"[MotionCapture] Copied {copied_count} workflow file(s) to {workflow_dir}")
        if skipped_count > 0:
            print(f"[MotionCapture] Skipped {skipped_count} existing workflow file(s)")

    except Exception as e:
        print(f"[MotionCapture] Error copying workflows: {e}")
