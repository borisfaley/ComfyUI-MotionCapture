...
class Patchifier(nn.Module):
    ...
    def forward(self, images, patches_per_image=80, disps=None, centroid_sel_strat='RANDOM', return_color=False):
        ...
        grid, _ = coords_grid_with_index(disps, device=device)
...
class VONet(nn.Module):
    ...
    def forward(self, images, poses, disps, intrinsics, M=1024, STEPS=12, P=1, structure_only=False, rescale=False):
        ...
        kk, jj = flatmeshgrid(torch.where(ix < 8)[0], torch.arange(0,8, device=device), indexing='ij')
        ii = ix[kk]

        imap = imap.view(b, -1, DIM)
        net = torch.zeros(b, len(kk), DIM, device=device, dtype=torch.float)
        ...
        while len(traj) < STEPS:
            ...
            if len(traj) >= 8 and n < images.shape[1]:
                ...
                kk1, jj1 = flatmeshgrid(torch.where(ix  < n)[0], torch.arange(n, n+1, device=device), indexing='ij')
                kk2, jj2 = flatmeshgrid(torch.where(ix == n)[0], torch.arange(0, n+1, device=device), indexing='ij')

                ii = torch.cat([ix[kk1], ix[kk2], ii])
                jj = torch.cat([jj1, jj2, jj])
                kk = torch.cat([kk1, kk2, kk])

                net1 = torch.zeros(b, len(kk1) + len(kk2), DIM, device=device)
