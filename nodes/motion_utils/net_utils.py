...
def detach_to_cpu(in_dict):
    ...
    if isinstance(in_dict, torch.Tensor):
        return in_dict.detach().cpu()
    elif isinstance(in_dict, dict):
        return {k: detach_to_cpu(v) for k, v in in_dict.items()}
    elif isinstance(in_dict, (list, tuple)):
        return type(in_dict)(detach_to_cpu(v) for v in in_dict)
    return in_dict
